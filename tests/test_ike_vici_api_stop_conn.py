...
def test_ipsec_ikeviciapi_conn_stop(topology):
    """
    Stop an active connection  using the IKEViciApi
    """
    sw1, sw2 = get_sw_from_topology(topology)

    sw1, sw2 = setup_sw1_sw2(sw1, sw2)

    # Wait 5 seconds for network interfaces
    sleep(5)

    sw1('ping -c 5 {0}'.format(ip_sw2), shell='bash_swns')
    # ping to sw1
    ping = sw2('ping -c 5 {0}'.format(ip_sw1), shell='bash_swns')
    ...
    sw1, sw2 = create_connection_using_strongswan(sw1, sw2, psk)
