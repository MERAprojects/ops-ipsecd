...
def test_ipsec_ikeviciapi_conn_delete(topology):
    """
    Delete a strongSwan connection  using the IKEViciApi
    """
    ...
    create_connection_using_strongswan(sw1, sw2, psk)
    ...
    # stop connection using the IKEViciApi
    stop = sw1(
            '{0} stop {1} 5000'.format(conn_cmd, conn_name),
            shell='bash_swns')
    assert 'Done' in stop

    # Check if there is any active connection using ipsec statusall
    status = sw1('ipsec statusall', shell='bash_swns')
    assert 'Security Associations (0 up, 0 connecting)' in status

    temp_cmd = '{0} delete {1}'.format(conn_cmd, conn_name)
    # delete a connection using the IKEViciApi
    delete = sw1(temp_cmd, shell='bash_swns')
    assert 'Connection deleted' in delete
    # Check if there is any active connection using ipsec statusall
    status = sw1('ipsec statusall', shell='bash_swns')
    assert 'Security Associations (0 up, 0 connecting)' in status
