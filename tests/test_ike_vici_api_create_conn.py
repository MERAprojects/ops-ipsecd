...
def test_ipsec_ikeviciapi_create_conn(topology):
    """
    Create a strongSwan connection  using the IKEViciApi
    """
    local = 'local'
    remote = 'remote'
    create_params_sw1 = 'name {0} ip {1} {2} id {3} {4} mode tunnel \
            integrity sha1 authby psk ike v1 cipher aes aes authmode esp \
            dgroup 2 cert none none'.format(
                    conn_name, ip_sw1, ip_sw2, local, remote)

    create_params_sw2 = 'name {0} ip {1} {2} id {3} {4} mode tunnel \
            integrity sha1 authby psk ike v1 cipher aes aes authmode esp \
            dgroup 2 cert none none'.format(
                    conn_name, ip_sw2, ip_sw1, remote, local)
    ...
    # create a connection using the IKEViciApi
    create = sw1(
            '{0} create {1}'.format(conn_cmd, create_params_sw1),
            shell='bash_swns')
    assert 'Connection created' in create

    # create a connection using the IKEViciApi
    create = sw2(
            '{0} create {1}'.format(conn_cmd, create_params_sw2),
            shell='bash_swns')
    assert 'Connection created' in create
    ...
    # start connection using the IKEViciApi
    start = sw1(
            '{0} start {1} {2}'.format(conn_cmd, conn_name, timeout_m),
            shell='bash_swns')
